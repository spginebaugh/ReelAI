...
import {
  convertJsonToSRT,
  convertJsonToVTT,
  validateSubtitleFormat,
} from "../services/subtitle-converter";
...
export const onAudioUploaded = onObjectFinalized({
  ...
  try {
    ...
    try {
      ...
      // Convert WAV to MP3
      await convertWavToMp3(tempInputPath, tempOutputPath);
      ...
      const srtContent = convertJsonToSRT(transcript);
      const vttContent = convertJsonToVTT(transcript);

      // Validate subtitle formats
      if (!validateSubtitleFormat(srtContent, "srt")) {
        throw new Error("Generated SRT content failed validation");
      }
      if (!validateSubtitleFormat(vttContent, "vtt")) {
        throw new Error("Generated VTT content failed validation");
      }

      // Calculate the base subtitles path
      const baseSubtitlesPath = event.data.name.replace(
        "/audio/audio_english.wav",
        "/subtitles/subtitles_english",
      );

      // Upload all subtitle formats
      const uploads = [
        // JSON subtitles
        bucket.file(`${baseSubtitlesPath}.json`).save(
      ...
      logger.info(
        "✅ Successfully generated and uploaded all subtitle formats:",
        {
